...
\end{flushright}
	...
	\section{Introduction}
		...
		Most of the prior work in this field is focused on generating natural language descriptions from images. The past two years have seen a lot of breakthroughs in image captioning problems but so far these have not been extended to the problem of  generating descriptions from videos.\newline
		Our aim is to utilize these new techniques in the field of image captioning and generate natural language descriptions from video data. Basically this project will involve first encoding video data using CNNs and then using RNNs (specifically LSTMs) to generate descriptions of these videos. Further, we will also explore using attention and optical flow based models to improve description of videos.
	...
	\section{Objectives}
		...
		\begin{enumerate}
			\item
				To organize all available sources of data and split them into training, validation and testing sets.
			\item
				To set up CNNs with pretrained weights for image captioning.
			\item
...
\section{Basic Concepts} 
    ...
    $\mathbf{\sigma_{g}}$, $\mathbf{\sigma_{c}}$ and $\mathbf{\sigma_{h}}$: Activation functions\\
			...
			\subsection{Training Deep Neural Networks}
				...
				\end{figure}
				A Deep Neural Network is at it's core a parameter based function. All of these parameters are  \textbf{trained} automatically from inputs and expected output tuples (training data). The training process revolves around minimizing a particular cost function using methods like \textbf{Stochastic gradient descent}. The input is given to the network in a feed forward fashion and the parameters are modified from the last layer to the first \textbf{(Propagation)}. Neural Networks, by design, require huge amounts of training data and take a large time to get trained. For some perspective, most current state of the art image classifiers have $> 100$ million parameters and are trained on more than 1.2 million images. 
	...
	\section{Approach to the project}
		First, we shall be implementing image captioning in order to be able to encode individual video frames in fixed-length vector representation.
		...
		\begin{enumerate}
			...
			\begin{enumerate}
				\item
					Construct a VGG/Inception V3 with pre-trained weights for object classification.
				\item
					Make an LSTM network\cite{showandtell} with CNN encoded image as input that will be used to translate the image representation into natural language text.
				\item
