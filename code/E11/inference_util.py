...
class Inference(object):
    ...
    def generate_caption_batch_beam(self, sess, beam_size,video_batch,max_len=20):  #  modifying this
        assert beam_size >= 2
        input_batch = np.array([[self.word_to_idx["<bos>"]]]*video_batch.shape[0])
        ...
        for i in range(max_len):
            ...
            for vv in range(len(batch_of_beams[0])):
                ...
                # want state  = ( 2, ?, 6000) (first 2000 are first layer (c and h) next are 2nd layer etc)
