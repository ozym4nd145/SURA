...
class Model_S2VT(object):
    def __init__(self,num_frames,
        ...
        self.num_layers_per_rnn = num_layers_per_rnn
    ...
    def build_embeddings(self):
        ...
        with tf.variable_scope("rnn_units"):
            self.cell_1 = tf.contrib.rnn.MultiRNNCell(
                    [tf.contrib.rnn.BasicLSTMCell(
                    self.hidden_size_lstm1,
                    reuse=tf.get_variable_scope().reuse) for _ in range(self.num_layers_per_rnn)])
            self.cell_2 = tf.contrib.rnn.MultiRNNCell(
                    [tf.contrib.rnn.BasicLSTMCell(
                    self.hidden_size_lstm2,
                    reuse=tf.get_variable_scope().reuse) for _ in range(self.num_layers_per_rnn)])
    ...
    def build_train_op(self):
        with tf.name_scope("Train"):
            ...
            with tf.name_scope("Decoding_state"):
                ...
                padded_caption = tf.zeros([batch_size_tensor,self.num_caption_unroll,self.num_last_layer_units]);
    ...
    def build_inference_op(self):
        with tf.name_scope("Inference"):
            ...
            with tf.name_scope("Video_Encoding"):
                ...
                self.infer_encoding_input_1 = self.rnn_input
                ...
                self.infer_hidden_state_1 = last_state_l1
                self.infer_hidden_state_2 = last_state_l2
                # print("len is **---- ",len(self.infer_hidden_state_1))    
                # print("shape is **---- ",self.infer_hidden_state_1)

            size_1_tuple_list = self.cell_1.state_size
            size_2_tuple_list = self.cell_2.state_size
            self.state_feed_1 = tf.placeholder(dtype=tf.float32,
                                          shape=[None, self.num_layers_per_rnn * 2 * self.hidden_size_lstm1],
                                          name="state_feed_1")
            self.state_feed_2 = tf.placeholder(dtype=tf.float32,
                                          shape=[None, self.num_layers_per_rnn * 2 * self.hidden_size_lstm2],
                                          name="state_feed_2")

            with tf.name_scope("Decoding_step"):
                state_tuple_1 = tf.split(value=self.state_feed_1,
                                    num_or_size_splits=self.num_layers_per_rnn, axis=1)
                state_tuple_2 = tf.split(value=self.state_feed_2,
                                    num_or_size_splits=self.num_layers_per_rnn, axis=1)
                state_tuple_1 = tuple([tf.contrib.rnn.LSTMStateTuple(*tf.split(
                                value=LayerTuple,
                                num_or_size_splits=2, axis=1)
                                ) for LayerTuple in state_tuple_1])
                state_tuple_2 = tuple([tf.contrib.rnn.LSTMStateTuple(*tf.split(
                                value=LayerTuple,
                                num_or_size_splits=2, axis=1)
                                ) for LayerTuple in state_tuple_2])
                
                # state_tuple_2 = tuple([tf.contrib.rnn.LSTMStateTuple(LayerTuple[:,0],LayerTuple[:,1]) for LayerTuple in state_tuple_2])
               
                # print("--",len(state_tuple_1),state_tuple_1[0],state_tuple_1[1],state_tuple_1[2])


                batch_size_tensor = tf.shape(self.word_encoded)[0]
                image_padding = tf.zeros([batch_size_tensor,1,self.num_last_layer_units])
                with tf.variable_scope("RNN_1",reuse=True) as scope:
                    outputs_l1, last_state_l1 = tf.nn.dynamic_rnn(
                                                cell=self.cell_1,
                ...
                self.infer_last_state_l1 = last_state_l1
                self.infer_last_state_l2 = last_state_l2
