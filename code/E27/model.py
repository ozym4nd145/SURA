...
class Caption_Model(object):
    ...
    def build_output_logits(self):
        # converting to shape (batch_size*num_frames,hidden_size_lstm)
        ...
        with tf.variable_scope("word_decoding") as scope:
            train_logits_decoded_to_embedding = tf.contrib.layers.fully_connected(inputs=train_input,
                                                          num_outputs=self.word_embedding_size,
                                                          activation_fn = None,
                                                          weights_initializer=self.initializer,
                                                          biases_initializer=self.initializer,
                                                          scope = scope)
            scope.reuse_variables()
            infer_logits_decoded_to_embedding = tf.contrib.layers.fully_connected(inputs=infer_input,
                                                          num_outputs=self.word_embedding_size,
                                                          activation_fn = None,
                                                          weights_initializer=self.initializer,
                                                          biases_initializer=self.initializer,
            ...
            self.full_embedding_inverse = tf.py_func(pseudo_inv, [self.full_embedding], tf.float32)

            train_logits_decoded = tf.matmul(train_logits_decoded_to_embedding,self.full_embedding_inverse)
            infer_logits_decoded = tf.matmul(infer_logits_decoded_to_embedding,self.full_embedding_inverse)
