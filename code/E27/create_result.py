...
def create(outfile_name,input_result_file,data_config):
    video_to_id_file = os.path.join(data_config["result_dir"],"video_to_id.json")
    gen_file = os.path.join(data_config["result_dir"],input_result_file)
    ...
    if not os.path.isfile(gen_file):
        print("%s not found in %s\nExiting" %(input_result_file,data_config["result_dir"]))
        sys.exit()
    ...
    for data in gen:
        ...
        dictionary["caption"] = data["gen_caption"][0][1]
        if is_ascii(dictionary['caption']):
            generated.append(dictionary)

    with open(os.path.join(data_config["result_dir"],outfile_name),"w") as fl:
        fl.write(json.dumps(generated,sort_keys=True,indent=4))

if __name__ == "__main__":
    data_config = configuration.DataConfig().config
    create("generated.json","generated_caption.json",data_config)
