...
def generate_captions(sess,model,video_path,infer_util,inception_checkpoint,
    ...
    if len(frame_list) != 0:
        print("Processing batch %d of %d" %(batch_index, num_batches))
        processed_batch = np.squeeze(np.array(frame_list,dtype=np.float32),axis=1)
        embedded_frames = np.reshape(processed_batch,[-1,num_frames_per_clip,inception_base.num_end_units_v4])
        caption_batch = infer_util.generate_caption_batch(sess,embedded_frames,max_len=max_cap_len)
        for cap in caption_batch:
            caption = {}
...
def main():
    ...
    captions = generate_captions(sess,model,video_path,infer_util,inception_checkpoint,
                                num_frames_per_clip,num_frames_per_sec,max_cap_len,batch_size)
