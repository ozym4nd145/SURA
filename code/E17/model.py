    ...
    def build_embeddings(self):
        with tf.device('/cpu:0'):
            ...
            with tf.variable_scope("embedding") as scope:
                ...
                self.word_emb_init = tf.constant(np.load(self.embedding_file),
                                            shape=[self.vocab_size-4, self.word_embedding_size],
                                            dtype=tf.float32,
                                            verify_shape=True)
                self.word_emb = tf.get_variable('glove_embedding', initializer=self.word_emb_init)

                # self.word_emb = tf.placeholder(dtype=tf.float32, name="word_embedding",
                #                                 shape=[self.vocab_size-4, self.word_embedding_size]
                #                                 )
                self.full_embedding = tf.concat([self.special_emb,self.word_emb],axis=0, name="word_embedding")
