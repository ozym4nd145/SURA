...
class Model_S2VT(object):
    ...
    def build_embeddings(self):
        with tf.variable_scope("inputs") as scope:
            self.rnn_input = tf.placeholder_with_default(self.inception_output,
                                                           [None,self.num_frames,self.num_last_layer_units],
                                                           name="rnn_input")
    ...
    def build_train_op(self):
        with tf.name_scope("Train"):
            ############################## TRAINING GRAPH #############################################
            ## Concatenate zeros to make input_rnn1 of shape = (batch_size,num_frames+num_caption_word,image_embedding_size)
            batch_size_tensor = tf.shape(self.image_encoded)[0]

            inputs_l1 = tf.concat([self.image_encoded,
                                    tf.zeros([batch_size_tensor,self.num_caption_unroll,self.image_embedding_size])],
                                    axis=1, name="padding_image_input")
            with tf.variable_scope("RNN_1") as scope:
                outputs_l1, last_state_l1 = tf.nn.dynamic_rnn(
                                            cell=self.cell_1,
                                            inputs=inputs_l1,dtype=tf.float32)
            self.train_input_1 = inputs_l1
            self.train_output_1 = outputs_l1
            #outputs_l1 is of shape (batch_size,num_frames+self.num_caption_unroll,self.hidden_size_lstm1)        
            caption_encoded = self.caption_encoded

            #caption_encoded is of shape(batch_size,self.num_caption_unroll+1,self.word_embedding_size)
            
            caption_needed = caption_encoded[:,:-1,:] #last word of caption is not relevant for input in rnn
            padded_caption = tf.concat([tf.zeros([batch_size_tensor,self.num_frames,self.word_embedding_size]),
                                        caption_needed],
                                       axis=1, name="padding_caption_input")
            #padded_caption should be of shape(batch_size,num_frames+self.num_caption_unroll,word_embedding_size)
            inputs_l2 = tf.concat([padded_caption,outputs_l1],axis=2)
            #inputs_l2 should be of shape(batch_size,num_frames+self.num_caption_unroll,word_embedding_size+hidden_size_lstm1)
            ...
            self.train_input_2 = inputs_l2
            self.train_output_2 = outputs_l2
            
            #outputs_l2 is of shape (batch_size,num_frames+self.num_caption_unroll,self.hidden_size_lstm2)
            self.train_last_state_l1 = tf.concat(last_state_l1,axis=1,name="last_state_1_concat_train")
            self.train_last_state_l2 = tf.concat(last_state_l2,axis=1,name="last_state_2_concat_train")
            self.train_output_rnn = outputs_l2[:,self.num_frames:,:]
            ############################################################################################
    ...
    def build_loss(self):
        with tf.variable_scope("loss") as scope:
            ...
            self.loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=correct_predictions,logits=self.train_logits)
            ...
            self.correct_loss = tf.multiply(self.loss, self.caption_mask) #shape = (batch_size,num_caption_unroll)
            
            batch_size_tensor = tf.shape(self.correct_loss)[0]

            self.individual_loss = tf.reduce_sum(self.correct_loss,axis=1) #shape = (batch_size,)
            
            #### Which loss to use?
            ## Sum of loss of all the words in a sequence (Done in s2vt code that we saw)
            #self.batch_loss = tf.reduce_sum(self.correct_loss)/tf.to_float(batch_size_tensor)
            ...
            self.batch_loss = tf.div(tf.reduce_sum(self.correct_loss),
